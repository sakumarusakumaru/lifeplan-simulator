    ...
    >
      <div className="flex flex-col gap-6">
        ...
        <SubGroup title="年金">
          <div className="grid grid-cols-1 gap-2">
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-1 md:grid-cols-2">
              <NumberField label="本人 受給開始" value={plan.penStartA} onChange={(v) => setField("penStartA", v)} unit="歳" />
              <NumberField label="本人 月額(手取)" value={plan.penAmtA} onChange={(v) => setField("penAmtA", v)} unit="円" />
            </div>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-1 md:grid-cols-2">
              <NumberField label="配偶者 受給開始" value={plan.penStartB} onChange={(v) => setField("penStartB", v)} unit="歳" />
              <NumberField label="配偶者 月額(手取)" value={plan.penAmtB} onChange={(v) => setField("penAmtB", v)} unit="円" />
            </div>
          </div>
          <div className="mt-3">
            <CheckboxField
